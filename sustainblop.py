...
def create_dataset(dataset, time_step=1):
    """Generates X and y datasets from a time-series dataset.
       Supports multiple features.
    """
    X, Y = [], []
    for i in range(len(dataset) - time_step - 1):
        X.append(dataset[i:(i + time_step)])
        Y.append(dataset[i + time_step, 3])  # Predict 'Close' column (assuming 0: Open, 1: High, 2: Low, 3: Close, 4: Volume)
...
def predict_future(model, data, days_to_predict, time_step):
    ...
    for _ in range(days_to_predict):
        ...
        # Update the window: append the predicted value while shifting out the oldest time step.
        # For simplicity, we copy the last window and update only the 'Close' feature.
        next_row = last_window[0, -1].copy()
        next_row[3] = next_pred  # Update only the 'Close' position
...
def load_model_and_data(tickers, esg_data):
    ...
    models = {}
    accuracies = {}
    # ESG scores as dictionary
    esg_scores = esg_data.set_index('ticker')['total_score'].to_dict()

    for ticker in tickers:
        try:
            # Downloading multiple features: Open, High, Low, Close, Volume
            ...
            if not stock_data.empty:
                ...
                # Split data (e.g., 65% training, 35% testing)
                ...
                # Update input shape based on number of features (here, 5)
                ...
                # Build a more robust model (using GRU here as an alternative)
                model = Sequential()
                ...
                )[:, 3]  # Extract the inverse-transformed 'Close'
        ...
        for ticker in tickers:
            if ticker in scaled_data:
                ...
                actual_predictions = scalers[ticker].inverse_transform(
                    np.hstack([scaled_data[ticker][-time_step:, :3],
                               future_predictions,
                ...
                plt.plot(data[ticker][:, 3], label=f"{ticker} Actual")  # 'Close'
                ...
                if ticker in accuracies:
                    st.write(f"Accuracy metrics for {ticker}: RMSE = {accuracies[ticker]['RMSE']:.2f}, MAE = {accuracies[ticker]['MAE']:.2f}")
            else:
